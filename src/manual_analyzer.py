...
for i in range(0,len(husten_minute)):
    ...
    energys_min_max.append( (np.max(sumdiff)-np.min(sumdiff)) / np.std(sumdiff))
    ...
    f.suptitle("Timecode: {}:{}. Husten: {}. Thresh: {}".format(h_min, h_sec, flag_husten[i], distance))


for idx,fg in enumerate(flag_husten):
    print("Husten: {}. Distance: {}. EnergyMinMax: {}".format(fg, distances[idx], energys_min_max[idx]))
